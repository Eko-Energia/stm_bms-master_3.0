...
/* Pack limits, from the BMSMaster_MasterVoltCurrTemp signal ranges. They key on
   ...
   JKP_DECIAMPS_MAX is the same number, so the decoder rejects any frame past
   this current outright: the test is >= rather than > so the limit is
   reachable at all. The JK also reports over-current itself, with its own
   configured delay, as 0x8b bits 5 and 6 in JK_StatusFlags bit 4 - this one
   has no debounce and judges a single 1 Hz sample. */
#define PACK_VOLT_MIN_CV    (6300u)    /* 63.0 V */
#define PACK_VOLT_MAX_CV    (8700u)    /* 87.0 V */
#define PACK_CURRENT_MAX_DA (3500)     /* 350.0 A, either direction */
...
static void evaluatePackLimits(void)
{
    ...
    const int16_t da = data.packDeciamps;
    if (da >= PACK_CURRENT_MAX_DA || da <= -PACK_CURRENT_MAX_DA) {
